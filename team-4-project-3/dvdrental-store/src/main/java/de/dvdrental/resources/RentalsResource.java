...
public class RentalsResource extends Resource {
    ...
    public Response searchRentals(@PathParam("searchValue") String search,
                                  @QueryParam("limit") @Max(100) @DefaultValue(value = "50") Integer limit,
                                  @QueryParam("offset") @DefaultValue("0") Integer offset,
                                  @Context UriInfo uriInfo) {
        List<RentalGetProxy> proxies = rentalRepository.searchAll(search, limit, offset).stream().map(rental -> {
            ...
            Href store = new Href(uriInfo.getBaseUriBuilder()
                    ...
                    .build());
            return new RentalGetProxy(customer, film, rental.getRentalDate(), rental.getRentalId(), rental.getReturnDate(), store);
    ...
    @GET
    public Response getRentals(@QueryParam("limit") @Max(50) @DefaultValue(value = "50") Integer limit,
                               @QueryParam("offset") @DefaultValue("0") Integer offset,
                               @Context UriInfo uriInfo) {
        List<RentalGetProxy> proxies = rentalRepository.getAll(limit, offset).stream().map(rental -> {
            Href customer = customerUri.map(s -> new Href(UriBuilder.fromPath(s)
                    .path("customers")
                    .path(Integer.toString(rental.getCustomerId()))
                    .build())).orElseGet(Href::new);
    ...
    @GET
    @Path("/count")
    public Long countRentals(@Context UriInfo uriInfo) {
    ...
    @Operation(summary = "Get rentals of customer")
    @APIResponse(responseCode = "200", description = "Rentals of customer with {id}", content = @Content(schema = @Schema(implementation = RentalGetProxy[].class)))
    @GET
    @Path("/customer/{id}")
    public Response getRentalsOfCustomer(@PathParam("id") Integer id, @Context UriInfo uriInfo) {
        List<RentalGetProxy> proxies = rentalRepository.getRentalsOfCustomer(id).stream().map(rental -> {
            Href customer = customerUri.map(s -> new Href(UriBuilder.fromPath(s)
                    .path("customers")
                    .path(Integer.toString(rental.getCustomerId()))
                    .build())).orElseGet(Href::new);
