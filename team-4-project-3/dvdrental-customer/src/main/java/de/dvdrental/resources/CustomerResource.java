...
public class CustomerResource extends Resource {
    ...
    @Operation(summary = "Searches id, firstName and lastName of customers by the search value.")
    @APIResponse(responseCode = "200", description = "Get customers that meet the search string.")
    @GET
    @Path("/search/{searchValue}")
    public Response searchCustomers(@PathParam("searchValue") String search,
                                    @QueryParam("limit") @Max(100) @DefaultValue(value = "100") Integer limit,
                                    @QueryParam("offset") @DefaultValue("0") Integer offset,
                                    @Context UriInfo uriInfo) {
        List<CustomerProxy> customerProxies = customerRepository.searchAll(search, limit, offset).stream().map(customer -> {
            URI address = uriInfo.getBaseUriBuilder()
                    .path("addresses")
                    .path(Integer.toString(customer.getAddress().getAddressId()))
                    .build();
            ...
            Href store = storeUri.map(s -> new Href(UriBuilder.fromPath(s)
                    ...
                    .path(Integer.toString(customer.getStoreID()))
                    .build())).orElseGet(Href::new);
            return new CustomerProxy(customer.getActive(), customer.isActivebool(), new Href(address),
                    customer.getCreateDate(), customer.getEmail(), customer.getFirstName(), customer.getCustomerId(), customer.getLastName(), store);
        }).collect(Collectors.toList());
        return createOkResponse(customerProxies);
    }
    @Operation(summary = "Get all customers")
    @APIResponse(responseCode = "200", description = "Get all customers")
    @GET
    public Response getAllCustomers(@QueryParam("limit") @Max(100) @DefaultValue(value = "100") Integer limit,
                                    @QueryParam("offset") @DefaultValue("0") Integer offset,
                                    @Context UriInfo uriInfo) {
        List<CustomerProxy> proxies = customerRepository.getAll(limit, offset).stream().map(customer -> {
            URI address = uriInfo.getBaseUriBuilder()
                    .path("addresses")
                    .path(Integer.toString(customer.getAddress().getAddressId()))
                    .build();
