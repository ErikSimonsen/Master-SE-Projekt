...
public class PaymentResource extends Resource {
    ...
    @Operation(summary = "Get sum of payments of corresponding rental {id}")
    @APIResponse(responseCode = "200", description = "sum of payments of corresponding rental ")
    @GET
    @Path("/sum/rental/{id}")
    public BigDecimal getSumPaymentsOfRental(@PathParam("id") Integer rentalId) {
        BigDecimal sum =  paymentRepository.getPaymentsSumForRental(rentalId);
        if (sum == null)
